...
public class TetrisOldMechanicsSound {
	//main method
	...
	static class tetrisJComponent extends JComponent {
		...
		//create an arrayList for the keys pressed
		...
		public void animate() {
			...
			if(keys.contains(38) || keys.contains(87)) {
				for(int i = 0; i < objectPoints.size(); i++) {
					objectPoints.set(i, objectPoints.get(i).rotate(90, size));
				}
				setObjectPoints();
				if(intersect(objectPointTranslated)) {
					for(int i = 0; i < objectPoints.size(); i++) {
						objectPoints.set(i, objectPoints.get(i).rotate(270, size));
					}
					setObjectPoints();
				}
				for(point p: objectPointTranslated) {
			...
			//first check right
			if(keys.contains(39) || keys.contains(87)) {
				objectPoint.changeX(objectPoint.getX() + 1);
				setObjectPoints();
				if(intersect(objectPointTranslated)) {
					objectPoint.changeX(objectPoint.getX() - 1);
					setObjectPoints();
			...
			//then check left
			if(keys.contains(37) || keys.contains(65)) {
				objectPoint.changeX(objectPoint.getX() - 1);
				setObjectPoints();
				if(intersect(objectPointTranslated)) {
					objectPoint.changeX(objectPoint.getX() + 1);
	...
	public static void playMusic(String filepath) {
		try {
			...
			JOptionPane.showMessageDialog(null,"Cannot Open Sound File");
		}
	}
}
